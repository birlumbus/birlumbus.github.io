...
// vertical/depth field (96×4 samples on a strand with absorbing ends).
//
...
//
// Absorbing top/bottom boundaries: out-of-bounds vertical indices return 0,
// which acts as a fixed node. Waves reflect at the ends, creating natural
// standing-wave patterns that look good on a finite strand.
//
...
export class Chime {
  ...
  constructor({ xPercent, yCenterPercent, heightPercent, baseWidth = 26 }) {
    ...
    // Wave field: 96 vertical samples × 4 depth bands
    this.vSamples = 96;
    this.dBands = 4;
  ...
  resize(canvasW, canvasH, hitBuffer = 16) {
    const bandW = Math.min(canvasW, 960);
  ...
  advanceRipple(delta) {
    ...
    const D = this.dBands;
    const spread  = 0.31;
    const damping = Math.pow(0.962, delta * 30);
    const settling = Math.pow(0.91, delta * 30);
    ...
    for (let j = 0; j < D; j++) {
      ...
      for (let i = 0; i < N; i++) {
        ...
        const curr = a[idx];
        const prev = b[idx];
        // Vertical neighbors: return 0 at boundaries (absorbing fixed node)
        const up    = i > 0     ? a[rowOff + i - 1] : 0;
        const down  = i < N - 1 ? a[rowOff + i + 1] : 0;
        ...
        const vel  = curr - prev;
        const next = (curr + vel * damping + (avg - curr) * spread) * settling;
  ...
  strike(yCrossing, strength) {
    ...
    this.injectImpulse(yNorm, strength);
  ...
  render(ctx, chord, globalPhase, crossfade = 1, prevChord = null) {
    ...
    for (let i = 0; i < N; i++) {
      ripple[i] = a[midJ * N + i];
      const above = i > 0     ? a[midJ * N + i - 1] : 0;
      const below = i < N - 1 ? a[midJ * N + i + 1] : 0;
      grad[i] = below - above;
    }
    ...
    for (let i = 0; i < N; i++) {
      ...
      const rippleOffset = g * 2.8 + r * 1.8;
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (trough > 0.01) {
        const alpha = trough * 0.26 < 0.12 ? trough * 0.26 : 0.12;
      ...
      if (crest > 0.01) {
        ...
        const [cr, cg, cb] = blendColors(pos);
        const amount = crest * 0.9 < 0.34 ? crest * 0.9 : 0.34;
        const rb = cr + (1 - cr) * amount;
        const gb = cg + (1 - cg) * amount;
        const bb = cb + (1 - cb) * amount;
        const alpha = crest * 0.95 < 0.36 ? crest * 0.95 : 0.36;
        const lineW = 0.8 + (crest * 5 < 2.1 ? crest * 5 : 2.1);
    ...
    // Bright white segments on steep wave fronts, additive composite so they
    // accumulate toward white. Mirrors drawRippleHighlights across all depth bands.
    ctx.save();
    clipToRoundedStrand();
    ...
    for (let j = 0; j < D; j++) {
      ...
      for (let i = 0; i < N; i++) {
        ...
        const crest = h * 0.55 + gradient * 1.35 - 0.045;
        if (crest <= 0) continue;

        const t3 = i / (N - 1);
        const lineW = 1.1 + (crest * 12 < 2.4 ? crest * 12 : 2.4);
        const alpha = crest * 1.7 < 0.28 ? crest * 1.7 : 0.28;
        ctx.fillStyle = `rgba(255,255,255,${alpha})`;
