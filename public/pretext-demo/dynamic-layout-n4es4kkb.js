...
function projectChromeLayout(layout2, contentHeight) {
  domCache.page.className = layout2.isNarrow ? "page page--mobile" : "page";
...
function commitFrame(now) {
  ...
  projectChromeLayout(layout2, contentHeight);
  const bodyLines = [
    ...leftLines.map((line) => ({ ...line, className: "line line--left" })),
    ...rightLines.map((line) => ({ ...line, className: "line line--right" }))
  ];
  const textProjection = {
    pageWidth: layout2.pageWidth,
    pageHeight: layout2.pageHeight,
    headlineFont: layout2.headlineFont,
    headlineLineHeight: layout2.headlineLineHeight,
    headlineLines,
    creditLeft,
    creditTop,
    bodyFont: font,
