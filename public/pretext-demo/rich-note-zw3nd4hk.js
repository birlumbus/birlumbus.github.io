...
// pages/demos/rich-note.model.ts
...
function prepareRichInlineNote(specs = DEFAULT_RICH_NOTE_SPECS) {
  const classNames = specs.map((spec) => spec.kind === "chip" ? CHIP_CLASS_NAMES[spec.tone] : TEXT_STYLES[spec.style].className);
  const flow = prepareRichInline(specs.map((spec) => {
    if (spec.kind === "chip") {
      return {
        text: spec.label,
        font: CHIP_FONT,
...
function layoutRichInlineItems(prepared, maxWidth) {
  const lines = [];
...
function resolveRichNoteBodyWidth(viewportWidth, requestedWidth) {
  const maxBodyWidth = Math.max(BODY_MIN_WIDTH, Math.min(BODY_MAX_WIDTH, viewportWidth - PAGE_MARGIN * 2 - NOTE_SHELL_CHROME_X));
  return {
    bodyWidth: Math.max(BODY_MIN_WIDTH, Math.min(maxBodyWidth, requestedWidth)),
    maxBodyWidth
  };
}
function layoutRichNote(prepared, bodyWidth) {
  ...
  return {
    ...
    lineCount,
    lines,
    noteBodyHeight: lineCount === 0 ? LAST_LINE_BLOCK_HEIGHT : (lineCount - 1) * LINE_HEIGHT + LAST_LINE_BLOCK_HEIGHT,
...
var richInline = prepareRichInlineNote(DEFAULT_RICH_NOTE_SPECS);
var st = {
  events: {
    sliderValue: null
  },
...
function render() {
  ...
  if (st.events.sliderValue !== null)
    requestedWidth = st.events.sliderValue;
  const { bodyWidth, maxBodyWidth } = resolveRichNoteBodyWidth(viewportWidth, requestedWidth);
  const layout2 = layoutRichNote(richInline, bodyWidth);
  ...
  domCache.root.style.setProperty("--note-width", `${layout2.noteWidth}px`);
  domCache.root.style.setProperty("--note-content-width", `${bodyWidth}px`);
  domCache.noteBody.style.height = `${layout2.noteBodyHeight}px`;
  renderBody(layout2.lines);
}
